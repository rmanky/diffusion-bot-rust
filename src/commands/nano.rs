...
#[command(name = "nano", desc = "Create an image with Gemini 3.1 Flash (🍌)")]
...
async fn send_error_message(
    ...
    } else if let Err(e) = client.update_response(token).embeds(Some(&[embed])).await {
...
async fn nano(
...
) -> Result<(Vec<u8>, &'static str), NanoError> {
    ...
    let (image, tier_used) = generate_image(
        reqwest_client,
